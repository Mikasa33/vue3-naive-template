...
// 是否加密
const isCrypto: boolean = import.meta.env.VITE_USE_CRYPTO === 'true';
...
export function setLocal(key: string, value: unknown, expire: number | null = DEFAULT_CACHE_TIME) {
  ...
  const json = isCrypto ? encrypto(storageData) : JSON.stringify(storageData);
...
export function getLocal<T>(key: string) {
  ...
  if (json) {
    ...
    try {
      storageData = isCrypto ? decrypto(json) : JSON.parse(json);
    } catch {
      // 防止解析失败
    }
    if (storageData) {
